...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    w = window_size
    data = list(data)
    n_feat = len(data[0][0][0])
    for sentence, labels in data:
		### YOUR CODE HERE (5-20 lines)
        sen_len = len(sentence)
        flatten = [x for y in sentence for x in y]
        flatten = (w * start) + flatten + (w * end)
        entry = [(flatten[i*n_feat: (i+2*w+1)*n_feat], labels[i]) for i in range(sen_len)]
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embed_mat = tf.get_variable(name='embed_mat', initializer=tf.constant(
        self.pretrained_embeddings), dtype=tf.float32)
        embed = tf.nn.embedding_lookup(embed_mat, self.input_placeholder)
        embeddings = tf.reshape(embed, shape=[-1, self.config.n_window_features
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(name='W', shape=[self.config.n_window_features
            * self.config.embed_size, self.config.hidden_size], initializer
            =xavier(), dtype=tf.float32)
        b1 = tf.get_variable(name='b1', initializer=tf.constant(0, shape=[
            self.config.hidden_size], dtype=tf.float32))
        ...
        U = tf.get_variable(name='U', shape=[self.config.hidden_size,
                                             ...
                                             initializer=xavier(),
                                                dtype=tf.float32)
        b2 = tf.get_variable(name='b2', initializer=tf.constant(0, shape=
                [self.config.n_classes], dtype=tf.float32))
    ...
    def add_loss_op(self, pred):
        ...
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred, labels=
            self.labels_placeholder)
